...
class ModelTrainer:
    ...
    def prepare_data(self):
        ...
        # Estratégia: remover colunas não-numéricas, depois preencher NaN
        if self.X.isnull().sum().sum() > 0:
            ...
            # Remover colunas de texto (não podem ter mean())
            text_cols = self.X.select_dtypes(include=['object']).columns.tolist()
            if len(text_cols) > 0:
                print(f"     Removendo {len(text_cols)} colunas de texto: {text_cols}")
                self.X = self.X.drop(columns=text_cols)
            
            # Remover colunas com >50% NaN
            ...
            self.X = self.X.bfill().ffill().fillna(self.X.mean()).fillna(0)
        ...
        if y_series.isnull().sum() > 0:
            ...
            y_series = y_series.bfill().ffill().fillna(y_series.mean()).fillna(0)
        ...
        # ATUALIZAR feature_names APÓS remover colunas
