...
class ModelTrainer:
    ...
    def prepare_data(self):
        ...
        if self.X.isnull().sum().sum() > 0:
            print(f"  AVISO: Ainda existem {self.X.isnull().sum().sum()} valores faltantes nas features")
            self.X = self.X.fillna(self.X.mean())
        
        # Verificar e preencher dados faltantes no TARGET
        y_series = pd.Series(self.y)
        if y_series.isnull().sum() > 0:
            print(f"  AVISO: Existem {y_series.isnull().sum()} valores faltantes no target")
            self.y = y_series.fillna(y_series.mean()).values
        else:
