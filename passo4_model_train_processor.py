...
class ModelTrainer:
    ...
    def prepare_data(self):
        ...
        if self.X.isnull().sum().sum() > 0:
            print(f"  AVISO: Ainda existem {self.X.isnull().sum().sum()} valores faltantes")
...
def run_training_for_all():
    ...
    # Carregar datasets do Passo 3
    datasets_to_train = {}
    
    for dataset_name in config.DATASETS:
        dataset_path = os.path.join(config.DATA_DIR, f"{dataset_name}_features.csv")
        ...
        datasets_to_train[dataset_name] = df
    ...
    for dataset_name, df in datasets_to_train.items():
        # Determinar quais estratégias usar
        if dataset_name == 'nao_agregado':
            strategies_to_use = ['A1_Direta']  # Apenas A1 para não agregado
        else:
            strategies_to_use = ['A1_Direta', 'A2_PCA', 'A3_Interacao']  # A1, A2, A3 para agregados
